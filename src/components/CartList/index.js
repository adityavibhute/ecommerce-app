...
const CartList = () => {
    ...
    const price = products?.length && products?.reduce((item, product) => item + product.price, 0);

    return (
        <div className="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
            <div className="flex justify-start item-start space-y-2 flex-col ">
                <h1 className="text-3xl lg:text-4xl font-semibold leading-7 lg:leading-9  text-gray-800">Order #{Math.floor(Math.random()*90000) + 10000}</h1>
            </div>
            <div className="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch  w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
                <div className="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
                    <div className="flex flex-col justify-start items-start bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
                        ...
                        {
                            products?.length && products.map((product) => <div key={product.id} className="mt-4 md:mt-6 flex  flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full ">
                                <div className="pb-4 md:pb-8 w-full md:w-40">
                                    <img className="w-full hidden md:block" src={product.image} alt="dress" />
                                ...
                                <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full  pb-8 space-y-4 md:space-y-0">
                                    ...
                                    <div className="flex justify-between space-x-8 items-start w-full">
                                        <p className="text-base xl:text-lg leading-6">
                                            ${product.price} <span className="text-red-300 line-through"> ${product.price + 5}</span>
                                        </p>
                                        <p className="text-base xl:text-lg leading-6 text-gray-800">01</p>
                                        <p className="text-base xl:text-lg font-semibold leading-6 text-gray-800">${product.price}</p>
                                    </div>
                                </div>
                            </div>
                            )
                    ...
                    {
                        products?.length ? <div className="flex justify-center md:flex-row flex-col items-stretch w-full space-y-4 md:space-y-0 md:space-x-6 xl:space-x-8">
                            <div className="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 space-y-6   ">
                                ...
                                <div className="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                                    ...
                                    <div className="flex justify-between items-center w-full">
                                        ...
                                        <p className="text-base leading-4 text-gray-600">-${Math.round(price / 10)} (10%)</p>
                                    </div>
                                    <div className="flex justify-between items-center w-full">
                                ...
                                <div className="flex justify-between items-center w-full">
                                    ...
                                    <p className="text-base font-semibold leading-4 text-gray-600">${Math.round(price - (price / 10)) + 8}</p>
                                </div>
                            </div>
                        </div> : null}
                </div>
